...
inputPanel(
  ...
  numericInput(
    inputId = "factor_entropy",
    label = "Increase factor on time for each Kludge",
    value = 1.05,
    step = 0.1
  ) ,
...
game_results <- eventReactive(eventExpr = input$go , valueExpr = {
  ...
  #forgot to increase the entropy

  #array of players and reviews 
  ...
  #separate actions and strategies   
  ...
  while(cur_time < input$total_time){

  ...
  if(save_db){
    ...
    dbWriteTable(
      ...
      value = saida$resultados %>% 
        ...
        pivot_longer(
          cols = starts_with("complete_strategy_") | starts_with("payoff_") | starts_with("util_"),
          names_pattern = "(.*)_([0-9])$",
          names_to = c(".value","dev")
        ) %>% 
        separate(
          col = complete_strategy,
          into = c("strategy_dev", "strategy_rev", "strategy_meta")
        ) %>% 
        mutate(
          across(
            .cols = c(dev, payoff),
...
observeEvent( input$solve_all_no_memo , {
  ...
  all_params <- all_params_all %>% 
    ...
    #retirar agora
    filter(
      process_fraction_review_game %in% c(50),
      process_fraction_metareview_game == c(0),
      characteristics_kludges_harmful_game == "Moderately"
    ) %>%
    identity()

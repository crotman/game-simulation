...
simulate_core <- function(input, n_executions_game){
  ...
  results_game <- furrr::future_map2_dfr(
    ...
    .f = ~simulate_executions(values = .x, n_executions = n_game, combination = .y ),
    progress = TRUE,
    .options = furrr::furrr_options(seed = 222 )
  )
  
  # eqs_game <- solve_game(params = values_game, results = results_game)
  # 
  # eq_table_game <- eqs_game %>% 
  #   eq_tables(
  #     reduce.tables = FALSE,
  #     combine = 0
  #   )
  # 
  # eqs_tibble <- eq_table_game %>% 
  #   map2_df(
  #     .y = 1:length(.),
  #     .f = function(.x, .y){
  #       tibble(
...
simulate_all <- eventReactive(eventExpr = input$solve_all , valueExpr = {
  ...
  for (i in 1:nrow(all_params)){
    ...
    simulate_core(input = this_input, n_executions_game = n_executions_game() )    
